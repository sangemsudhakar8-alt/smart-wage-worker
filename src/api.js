...
export const sendOTP = async (phone, verifier) => {
    ...
    const isFirebaseKeySet = import.meta.env.VITE_FIREBASE_API_KEY && 
                             import.meta.env.VITE_FIREBASE_API_KEY !== 'your_api_key_here';
    
    // DEV / DEMO MODE: If Firebase credentials are not configured or cleanPhone is a standard 10-digit number
    if (!isFirebaseKeySet || cleanPhone.length === 10) {
        console.log("Using Mock OTP for Dev / Demo mode:", cleanPhone);
        return {
            confirm: async (otp) => {
    ...
    try {
        ...
        console.warn("Firebase SMS OTP send failed, falling back to Mock OTP mode:", err);
        return {
            confirm: async (otp) => {
                if (otp === "123456" || otp.length === 6) {
                    return { user: { uid: `mock_user_${cleanPhone}` } };
...
export const loginFirebaseUser = async (phone, role, userId) => {
    ...
    try {
        ...
        console.warn("Backend auth session endpoint offline, using fallback profile:", e);
    }
    
    // Fallback user profile for demo mode
    const cleanPhone = phone.replace(/\D/g, '');
    return {
        user: {
            id: userId || `mock_user_${cleanPhone}`,
            phone: phone,
            role: role || 'worker',
            name: role === 'employer' ? 'Demo Employer' : 'Demo Worker',
            location: 'Hyderabad, TS',
            token: `mock_token_${userId || cleanPhone}`
        }
    };
};
